...
spaceshipSelectHandle = async(e) => {
  ...
  if(this.state.shipInitialSelect === "1") {
    const color = await request.get(`http://www.whateverorigin.org/get?url=http://www.colr.org/json/scheme/11154`)
    ...
    const color = await request.get(`http://www.whateverorigin.org/get?url=http://www.colr.org/json/scheme/17822`)
    const parsedObject = JSON.parse(color.text);
    ...
    const color = await request.get(`http://www.whateverorigin.org/get?url=http://www.colr.org/json/scheme/7078`)
